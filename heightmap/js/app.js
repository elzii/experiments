var APP = (function () {
  ...
  app.init = function () {
    ...
    this.readImage('heightmap.jpg', function () {
    ...
    this.loadHeightmap( function() {
      ...
      var imgd = app.$el.ctx.getImageData(0, 0, 512, 512);
      var pix = imgd.data;
      ...
      app.$el.ctx.putImageData(imgd, 0, 0);
   ...
   * Draw Heightmap
   */
  app.loadHeightmap = function (callback) {
    
    var img = this.$el.heightmap;

    this.$el.ctx.drawImage(img, 0, 0, 512, 512)
   ...
   * Read Image
   */
  app.readImage = function (image, callback) {
    
    var image = image || null,
        image_path = window.location.href+image;

    // var reader = new FileReader()
    //     reader.onload = (function (img) {  console.log(event, img) })
    ...
    xhr.onload = function(e) {
      ...
      if (this.status == 200) {
        ...
        var blob      = this.response,
            Int8Array = new Uint8Array(this.response),
            img       = document.createElement('img')
        
        img.src = window.URL.createObjectURL(blob)
        img.setAttribute('id', 'heightmap')
