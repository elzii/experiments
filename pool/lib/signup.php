<?php

include '../db/config.php';

/*** begin our session ***/
session_start();

$data = [];

/*** first check that both the username, password and form token have been sent ***/
if(!isset( $_POST['username'], $_POST['password'], $_POST['form_token']))
{
    $data['message'] = 'Please enter a valid username and password';
    $data['state']   = 'ERROR';
}
/*** check the form token is valid ***/
elseif( isset($_POST['form_token']) != $_SESSION['form_token'])
{
    // $data['message'] = 'Token Error. POST form_token was '. $_POST['form_token'] .'but SESSION form_token was'. $_SESSION['form_token'] .'';
    $data['message'] = 'Token Error. POST form_token did not match SESSION form_token. Maybe session was unset?';
    $data['state']   = 'ERROR';
}
/*** check the username is the correct length ***/
elseif (strlen( $_POST['username']) > 20 || strlen($_POST['username']) < 4)
{
    $data['message'] = 'Incorrect Length for Username';
    $data['state']   = 'ERROR';
}
/*** check the password is the correct length ***/
elseif (strlen( $_POST['password']) > 20 || strlen($_POST['password']) < 4)
{
    $data['message'] = 'Incorrect Length for Password';
    $data['state']   = 'ERROR';
}
/*** check the username has only alpha numeric characters ***/
elseif (ctype_alnum($_POST['username']) != true)
{
    /*** if there is no match ***/
    $data['message'] = "Username must be alpha numeric";
    $data['state']   = 'ERROR';
}
/*** check the password has only alpha numeric characters ***/
elseif (ctype_alnum($_POST['password']) != true)
{
        /*** if there is no match ***/
        $data['message'] = "Password must be alpha numeric";
        $data['state']   = 'ERROR';
}
else
{
    /*** if we are here the data is valid and we can insert it into database ***/
    $username = filter_var($_POST['username'], FILTER_SANITIZE_STRING);
    $password = filter_var($_POST['password'], FILTER_SANITIZE_STRING);

    /*** now we can encrypt the password ***/
    $password = sha1( $password );
    

    try
    {
        $dbh = new PDO("mysql:host=$mysql_hostname;dbname=$mysql_dbname", $mysql_username, $mysql_password);
        /*** $data['message'] = a message saying we have connected ***/

        /*** set the error mode to excptions ***/
        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        /*** prepare the insert ***/
        $stmt = $dbh->prepare("INSERT INTO users (username, password ) VALUES (:username, :password )");

        /*** bind the parameters ***/
        $stmt->bindParam(':username', $username, PDO::PARAM_STR);
        $stmt->bindParam(':password', $password, PDO::PARAM_STR, 40);

        /*** execute the prepared statement ***/
        $stmt->execute();

        /*** unset the form token session variable ***/
        unset( $_SESSION['form_token'] );

        /*** if all is done, say thanks ***/
        $data['message']  = 'Welcome '. $username . '!';
        $data['state']    = 'SUCCESS';
        $data['username'] = $username;
    }
    catch(Exception $e)
    {
        /*** check if the username already exists ***/
        if( $e->getCode() == 23000)
        {
            $data['message'] = 'Username already exists';
            $data['state']   = 'ERROR';
        }
        else
        {
            /*** if we are here, something has gone wrong with the database ***/
            $data['message'] = 'We are unable to process your request. Please try again later"';
            $data['state']   = 'ERROR';
        }
    }
}

  echo json_encode($data);
?>